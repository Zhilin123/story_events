...
class TaggingDataHandler:
  ...
  def createModellingExamples(self,tok,max_len=512):
    ...
    for c in [tokCol]:
      ...
      nCols.append(nCol)
    #print(df.head())
    #print(df[labelCol].head())
    #print(set([type(df[labelCol][i]) for i in range(len(df[labelCol]))]))
    # realigning the labels
    df[labelCol+"_ids"] = df[labelCol].apply(
      lambda x: [[self.labelMap.get(i,0)] for i in x])
    #df[labelCol+"_ids"] = df[labelCol].apply(
    #  lambda x: [self.labelMap.get(x,0)])
...
if __name__ =="__main__":
  ...
  parser.add_argument("--txtCol", default=None)
